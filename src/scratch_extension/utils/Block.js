...
export class Block {
    ...
    serve_method;
    text;
    constructor(opcode, blockType, text = "", serve_method = "POST", args = undefined) {
        this.opcode = opcode;
    ...
    async requestHandler(url, request_method = 'POST', request_body = null) {
        try {
            const response = await fetch(`http://${config.robomaster_api.host}:${config.robomaster_api.port}/${url}`, {
